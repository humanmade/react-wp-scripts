...
const devServerConfig = require.resolve(
...
requireMiddleware.use((req, next) => {
  if (
    req.request.indexOf("../config/") === 0 &&
    req.path.indexOf("react-scripts/config") >= 0
  ) {
  ...
  if (Array.isArray(config.entry)) {
    ...
    if (hotClientIndex >= 0) {
      config.entry.splice(
        hotClientIndex,
        1,
        require.resolve("../overrides/webpackHotDevClient")
      );
    }
